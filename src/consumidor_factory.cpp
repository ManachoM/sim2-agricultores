...
Consumidor *ConsumidorFactory::create_consumidor(std::string const &cons_type, const int feria_id)
{
    Consumidor *cons;
    if (cons_type == "single")
    {
    ...
    else if (cons_type == "family_budget")
    {
        ...
        cons = new ConsumidorFamiliarPresupuesto(cant_integrantes, feria_id, budget, this->fel);
    }
    else
