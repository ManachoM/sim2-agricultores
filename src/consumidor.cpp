...
void Consumidor::process_init_compra()
{
  ...
  { //
    ...
    msg.insert(MESSAGE_KEYS::BUYER_ID, (double)this->get_consumer_id());
...
void Consumidor::process_resp_feriante(const Event *e, json &log)
{
  ...
  if (msg.find(MESSAGE_KEYS::ERROR) == -1) // Si retornamos con error
  {
  ...
  log["feriante_id"] = msg.find(MESSAGE_KEYS::SELLER_ID);
  this->finish_purchase();
}
