...
private:
  ...
  std::map<int, std::unordered_set<int>>
      feriante_producto; /** Matriz de booleanos que almacena el índice
                            invertido*/
  Environment *env;
...
public:
  ...
  Feria(
      std::vector<int> const &_dias, const int _num_feriantes,
      Environment *_env = nullptr, FEL *_fel = nullptr
  );

  std::map<int, Feriante *> get_feriantes() const;

  /**
   * @brief Devuelve lista de id's con todos los feriantes que tienen stock de
   * un producto
  ...
  std::vector<int> get_dia_funcionamiento() const;
};
