...
ProductPartioner::ProductPartioner(const std::map<int, Producto> &_prods) : prods(_prods)
{
}
...
double ProductPartioner::objective_function(std::vector<int> &_sol)
{
  ...
  // donde cada posición es un mes del año y el valor es la cantidad de productos
  // en ese procesador que se comercializan en dicho mes
  ...
  // Ahora, recorremos cada una de las entradas por procesador y vamos sumando la
...
std::vector<int> ProductPartioner::next_neighbour(const std::vector<int> &_sol, const int r, const int n_procs)
{
  int num_prods = (int) this->prods.size();

  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<int> prod_chooser(0, num_prods);
  std::uniform_int_distribution<int> proc_picker(0, n_procs);

  std::vector<int> changed_prods(num_prods, 0);
  // Elegimos los productos a ser modificados y les designamos un nuevo procesador
  for(int i = 0; i < r ; ++i)
  {
...
float ProductPartioner::acceptance_probability(double e1, double e2, double temp)
{
  if (e2 < e1)
    return 1.0;
...
std::unordered_map<int, std::vector<Producto>> ProductPartioner::partition_products(const int n_procs, const int k_max, const int r)
{
  ...
  // Generamos una solución inicial usando el módulo en función de la cantidad de 
  // procesadores
  ...
  do
  {
    ...
    // Si la probabilidad de aceptación es mayor a un random, actualizamos la mejor solución
    if(this->acceptance_probability(this->objective_function(best_sol), this->objective_function(neighbour), temp) >= rand_gen(gen))
      best_sol = neighbour;
    ++k;
  }
  while(k < k_max);
