...
private:
  ...
  std::vector<int>
  next_neighbour(const std::vector<int> &_sol, const int r, const int n_procs);
  float acceptance_probability(double e1, double e2, double temp);

public:
  ProductPartioner(const std::unordered_map<int, Producto *> &_prods);
  std::unordered_map<int, std::vector<Producto *>> partition_products(
      const int n_procs, const int k_max = 1'000, const int r = 1
  );
};
