...
AgricultorGanancia::AgricultorGanancia(FEL *_fel, Terreno *_terr, MercadoMayorista *_mer) : Agricultor(_fel, _terr, _mer)
{
}
...
const int AgricultorGanancia::choose_product()
{
    ...
    double best_ganancia = (chosen->get_rendimiento() * terr_size * chosen->get_precios_mes().at(month)) - terr_size - chosen->get_costo_ha();
