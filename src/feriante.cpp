...
void Feriante::process_init_compra()
{
  ...
  for (int prod_id : prod_ids)
  {
    ...
    //! Esto va comentado porque, en distribuido, no nos interesa
    // el id del agricultor - se maneja a nivel de proc
...
void Feriante::process_resp_agricultor(const Event *e, json &log)
{
  ...
  double prod_id = msg.find(MESSAGE_KEYS::PROD_ID);
  ...
  if (msg.find(MESSAGE_KEYS::ERROR) == -1)
  {
    log["id_agricultor"] = -1;
    // msg.msg.erase("error");
    msg.insert(MESSAGE_KEYS::ERROR, 0.0);
    Agricultor *new_agr;
  ...
  log["id_agricultor"] = msg.find(MESSAGE_KEYS::SELLER_ID);
...
void Feriante::process_venta_feriante(const Event *e)
{
  ...
  double prod_id = msg.find(MESSAGE_KEYS::PROD_ID);
  double amount = msg.find(MESSAGE_KEYS::AMOUNT);
  double buyer_id = msg.find(MESSAGE_KEYS::BUYER_ID);
  ...
  {
    msg.insert(MESSAGE_KEYS::ERROR, -1);
    this->fel->insert_event(
  ...
  msg.insert(MESSAGE_KEYS::SELLER_ID, (double)this->get_feriante_id());
  this->fel->insert_event(
