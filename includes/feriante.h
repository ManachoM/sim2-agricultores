...
private:
  ...
  std::map<int, Inventario>
      inventario; /** Mapa del tipo <id_producto, Inventario> con todo el
                     inventario del feriante */

  int feria_id;

  std::vector<int>
      productos; /** Arreglo con los productos que el feriante venderá*/

  virtual std::vector<int> choose_product() = 0;

  virtual double purchase_amount(const int prod_id) = 0;

  virtual Agricultor *choose_agricultor(const int prod_id,
  ...
  void process_compra_mayorista(const Event *e, json &log);

public:
  MercadoMayorista *mercado; /** Puntero al mercado mayorista del sistema*/
  Feriante(FEL *list = nullptr, MercadoMayorista *_mer = nullptr,
           int feria_id = -1);
  ...
  */
  ...
  /*!
      ...
      @return Inventario de ese producto que mantiene el feriante. Si el
     feriante no tiene inventario de ese producto, retorna una instancia de
     inventario con todo 0 excepto el id_producto.
  */
  Inventario get_inventario_by_id(int idProducto);

  void set_inventario_by_id(int id_prod, const Inventario &inv);
