...
BooleanMatrix::BooleanMatrix(int _num_rows, int _num_cols)
    : matrix(std::vector<std::vector<bool>>(_num_rows)), num_rows(_num_rows),
      num_cols(_num_cols) // matrix(new bool *[num_rows])
{
  for (int i = 0; i < num_rows; ++i)
  {
    ...
    // std::cout << "i: " << i << " num_rows: " << this->matrix.size() <<
    // std::endl; for (int j = 0; j < num_cols; ++j)
    // {
