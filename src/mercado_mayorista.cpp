...
MercadoMayorista::MercadoMayorista(Environment *_env)
    : env(_env){
...
void MercadoMayorista::update_index(int agr_id, int prod_id, bool inv)
{
  ...
  // std::cout << "MATRIX PTR: " << &this->prod_mat << " - DIMS: " <<
  // this->prod_mat.num_rows << " x " << this->prod_mat.num_cols << std::endl;
...
void MercadoMayorista::reset_index()
{
  ...
  std::cout << "agros " << agros.size() << "  prods: " << prods.size()
            << std::endl;
  ...
  // std::cout << "Dimensiones nueva matriz" << this->prod_mat.num_rows << " x "
  // << this->prod_mat.num_cols << "\n" << std::endl;
}
